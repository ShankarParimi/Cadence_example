...
func (h *SampleHelper) StartWorkflowWithCtx(ctx context.Context, options client.StartWorkflowOptions, workflow interface{}, args ...interface{}) (string, string) {
	...
	//h.Logger.Debug("Hey Just now its started or completed i dont know.")
	if err != nil {
...
func (h *SampleHelper) QueryWorkflow(workflowID, runID, queryType string, args ...interface{}) interface{} {
	workflowClient, err := h.Builder.BuildCadenceClient()
	if err != nil {
		h.Logger.Error("Failed to build cadence client.", zap.Error(err))
		panic(err)
...
func (h *SampleHelper) SignalWorkflow(workflowID string, signal string, data interface{}) {
	workflowClient, err := h.Builder.BuildCadenceClient()
	if err != nil {
		h.Logger.Error("Failed to build cadence client.", zap.Error(err))
		panic(err)
...
func GetCommonHelper() *SampleHelper {
	instance := getInstance()
	instance.SetupServiceConfig()
	return instance
}

func getInstance() *SampleHelper {
	if h == nil {
		h = &SampleHelper{} // <--- NOT THREAD SAFE
	}
	return h
}
